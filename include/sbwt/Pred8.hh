...
class Pred8 {
   public:
    ...
    Pred8(const vector<uint64_t> &data) {
       _n = data.size();
       _min = data[0];
       _u = data[data.size()-1] - _min;
       _nblocks = _u/256 + ((_u%256) > 1);

       _X = new uint32_t[_nblocks];
       ...
       cerr << "Pred8: _u _n _min _nblocks _nActiveBuckets: "<<_u<<' '<<_n<<' '<<_min<<' '<<_nblocks<<' '<<_nActiveBuckets<<'\n';
       cerr << "Pred8: sizeInBytes(): "<<sizeInBytes()<<'\n';
       uint64_t yi = 0;
    ...
    uint64_t sizeInBytes() const{
        uint64_t sz = 4*sizeof(uint64_t) + (sizeof(uint32_t)*_nblocks) + (sizeof(uint8_t)*_nblocks) + _n;
        return sz;
    }
    ...
    int64_t serialize(std::ostream& os) const{
        ...
        os.write((char *)_X,sizeof(uint32_t)*_nblocks);
    ...
    void load(std::istream& is){
       ...
       _X = new uint32_t[_nblocks];
       is.read((char *)_X, sizeof(uint32_t)*_nblocks);
       _C = new uint8_t[_nblocks];
