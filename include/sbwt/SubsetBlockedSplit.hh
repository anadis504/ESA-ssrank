...
class SubsetBlockedSplitRank {
 ...
 public:
  ...
  SubsetBlockedSplitRank() {}

  SubsetBlockedSplitRank(const sdsl::bit_vector& A_bits,
  ...
  SubsetBlockedSplitRank(const SubsetBlockedSplitRank& other) {
    assert(&other != this);  // What on earth are you trying to do?
    operator=(other);
  }

  SubsetBlockedSplitRank& operator=(const SubsetBlockedSplitRank& other) {
    if (&other != this) {
